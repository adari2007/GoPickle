...
```
VITE_API_BASE = https://gopickle-api-857090583357.us-west1.run.app
```
...
Cloud Run URL, not `localhost:4000`.

## Notes

- `web/public/_redirects` rewrites every path to `index.html` so client-side
  views survive a hard refresh or a deep link.
- The API scales to zero on Cloud Run's free tier, so the first request after
  a quiet period takes 1–5s. If that's annoying, hit `/health` on a schedule
  to keep it warm, or set `--min-instances=1` (moves it off the free tier).
...
the Cloud Run URL in `mobile/.env` before building with EAS.
